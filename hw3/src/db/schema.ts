...
export const actTable = pgTable(
  "activity",
  {
    id: serial("id").primaryKey(),
    title: varchar("title", { length: 280 }).notNull().unique(),
...
export const joinsTable = pgTable(
  "joins",
  {
    ...
    actId: integer("act_id")
      .notNull()
      .references(() => actTable.id, { onDelete: "cascade" }),
  },
  (table) => ({
    actIdIndex: index("tweet_id_index").on(table.actId),
    userHandleIndex: index("user_handle_index").on(table.userHandle),
    ...
    uniqCombination: unique().on(table.userHandle, table.actId),
  }),
);
